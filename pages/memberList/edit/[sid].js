...
import { API_SERVER_EDIT, API_ORIGINAL } from '@/components/config/api-path'
import { z } from 'zod'
...
export default function MBedit() {
  ...
  const handleSubmit = async (e) => {
    ...
    // 連線資料庫，新增資料
    const r = await fetch(`${API_SERVER_EDIT}/${router.query.user_id}`, {
      method: "PUT",
  ...
  useEffect(() => {
    ...
    fetch(`${API_ORIGINAL}/${router.query.user_id}`)
      .then(r => r.json())
      .then(result => {
        if (result.success) {
          const { name, email, birthday, created_at } = result.data
          setmbInfo({ name, email, birthday, created_at })
        }
        else {
          ...
          <div className="card">
            <div className="card-body">
              ...
              <form name="form1" method="post" onSubmit={handleSubmit}>
                ...
                <button type="submit" className="btn btn-primary">編輯</button>
